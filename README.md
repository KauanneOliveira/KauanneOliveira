...
Sou `Kauanne Oliveira`, uma estudante de `21 anos`, natural de `São Paulo - SP`, formada pelo `Instituto Federal de São Paulo (IFSP)` como `Técnica em Informática`. Atualmente, estou cursando o `Técnico em Desenvolvimento de Sistemas` na `Escola Senai Anchieta`, iniciando minha graduação em `Ciência da Computação` na `Faculdade de Informática e Administração Paulista (FIAP)`  e atuando como `Aprendiz no Metrô de São Paulo`.
